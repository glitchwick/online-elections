...
function loadData(records = []) {
	...
	for(let i=0; i<records.length; i++) {
		...
		table_data += `<td> ${records[i].election_id}</td>`;
		table_data += `<td> ${records[i].election_name}</td>`;
...
function postData() {
	var election_id = Number(document.getElementById("election_id").value);
	var election_name = String(document.getElementById("election_name").value);
	...
	fetch(api_url + 'elections/add-elections', {
		method: "POST",
		body: formData
	})
	.then((response) => response.json())
	.then((data) => { 
		...
		else
		{
			console.log(data); 
			alert('Success');
			window.location.href = "add-elections.html"; 
